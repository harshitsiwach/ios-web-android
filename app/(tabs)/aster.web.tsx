    ...
    >
      ...
      <View style={styles.section}>
        ...
        {ticker24hrData.slice(0, 5).map((ticker, index) => (
          <View key={index} style={styles.row}>
            <Text style={[styles.symbol, { color: colors.text }]}>{ticker.symbol}</Text>
            <Text style={[styles.price, { color: parseFloat(ticker.priceChangePercent) >= 0 ? '#4CAF50' : '#F44336' }]}>
          ...
          <View>
            ...
            {orderBook.bids.map((bid, index) => (
              <View key={`bid-${index}`} style={styles.row}>
                <Text style={[styles.bidPrice, { color: '#4CAF50' }]}>{bid[0]}</Text>
            ...
            {orderBook.asks.map((ask, index) => (
              <View key={`ask-${index}`} style={styles.row}>
                <Text style={[styles.askPrice, { color: '#F44336' }]}>{ask[0]}</Text>
